...
//Read sensor memory(handle to I2C, device address, register (from which //we want to read), 1 (1-byte register), pointer to location where data will be //read, we want to read 1 byte, timeout after 1 s)
//blocking function has timeouts
//Address shifted to the left by 1 in HAL standard
...
{
	 //the youngest byte that has the oldest position
	uint8_t Value[3];

	HAL_I2C_Mem_Read(bmp->bmp_i2c, ((bmp->Address)<<1), Register, 1, Value, 3, BMP280_I2C_TIMEOUT);

	//Instead, the last read goes to the youngest position
...
{
	uint8_t Tmp; //register ctrl_meas

	//check, if bigger set it to 3, truncate to 3 bits

	if(Mode > 3) Mode = 3;

	Tmp = Read8(bmp, BMP280_CONTROL); //read the entire register first

	Tmp = Tmp & 0xFC; // Tmp (xxxx xx00) //we only zero the last 2 bits
	Tmp |= Mode & 0x03; //we mask the two youngest bits

	Write8(bmp, BMP280_CONTROL, Tmp); //save and send back the new registry value
}
...
void BMP280_SetTemperatureOversampling(BMP280_t *bmp, uint8_t TOversampling)
{
	...
	if(TOversampling > 5) TOversampling = 5; //shortening to 5 bits

	Tmp = Read8(bmp, BMP280_CONTROL);

	Tmp = Tmp & 0x1F; // Tmp (000x xxxx) //set bits 2, 3 and 4
	//shifted by the previous 2 bits
...
float BMP280_ReadTemperature(BMP280_t *bmp)
{
	...
	//BMP documentation calculations
